...
export default class NameCommand extends BaseCommand {
  ...
  async run(base: TempContext) {
    if (base.flags.includes("all") || base.flags.includes("complete")) {
      var usuario =
        base.message.mentions.users.first() ||
        base.client.users.cache.get(base.args[0]);
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `Cuenta creada **${tiempo}** y el usuario lleva desde **${new TimeStamp(
            base.guild.members.cache.get(`${usuario.id}`).joinedTimestamp
