...
export default class SlashEval extends BaseSlashCommand {
  ...
  async run(base: SlashContext) {
    ...
    async function messages(id: string) {
      return interaction.channel.messages.fetch(id);
    }
    ...
    async function members(id?: string | number) {
      if (!id) return interaction.guild.members.cache.map((x) => x);
      return await interaction.guild.members.cache.get(String(id));
    }
