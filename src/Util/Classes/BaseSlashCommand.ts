...
export class BaseSlashCommand {
  ...
  canRun(int: CommandInteraction, isDev: boolean) {
    ...
    int.guild.members.fetch(`${int.member.id}`).then((x) => {
      if (this.dev == true) {
        if (this.devs.includes(int.member.id)) {
          return int.reply({
            embeds: [
              e.setDescription(
                `Lo siento pero debes de tener conocimientos de programación\npara poder ejecutar este comando.`
              ),
            ],
            ephemeral: true,
          });
      ...
      if (this.staff == true) {
        if (!x.roles.cache.has(`867293184216662086`)) {
          if (!x.roles.cache.has(`821724207816769597`)) {
            return int.reply({
              embeds: [
                e.setDescription(
                  `Lo siento pero debes de ser\n<@&821724207816769597> para poder ejecutar este comando.`
                ),
              ],
              ephemeral: true,
            });
