...
export async  function verificaction (interaccion) {
   ...
   await interaccion.guild.members.fetch(`${interaccion.member.id}`)
   ...
   setTimeout(() => {const a = ["871542385975386112", "820639213208862740", "871506101164703765", "871502962143395932", "871502816236175371", "871501359218851850", "851129161367420938"]; Promise.all(a.map(x => interaccion.guild.members.cache.get(`${interaccion.member.id}`).roles.add(`${x}`, `Roles al pasar la verificación`)));}, 10000)
